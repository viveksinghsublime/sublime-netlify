...
export async function GET(request, { params }) {
  try {
    ...
    const record = await getMasterById('role', Number(resolvedParams.id));
    if (!record) {
      return jsonError('Job role not found.', 404, { data: null });
    }
    return jsonSuccess(record, 'Job role fetched successfully.');
  } catch (error) {
...
export async function PUT(request, { params }) {
  try {
    ...
    const record = await updateMaster('role', Number(resolvedParams.id), payload);
    revalidateJobContent();
    return jsonSuccess(record, 'Job role updated successfully.');
  } catch (error) {
...
export async function DELETE(request, { params }) {
  try {
    requireAdminRequest(request);
    const resolvedParams = await params;
    await softDeleteMaster('role', Number(resolvedParams.id));
    revalidateJobContent();
    return jsonSuccess(null, 'Job role deleted successfully.');
  } catch (error) {
    return jsonError(error.message === 'UNAUTHORIZED' ? 'Unauthorized.' : error.message, error.message === 'UNAUTHORIZED' ? 401 : 400);
  }
}
